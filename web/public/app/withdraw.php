...
<body class="bg-slate-900 text-white min-h-screen pb-20">
  ...
  <main class="max-w-lg mx-auto px-4 py-5 space-y-5">
    ...
    <div class="bg-slate-800 rounded-2xl p-5 space-y-4">
      <h2 class="font-bold text-white text-base">Select Withdrawal Method</h2>

      <div class="grid grid-cols-2 gap-3">
        <button type="button" id="btnBank"
          onclick="selectMethod('bank')"
          class="method-card border-2 border-transparent rounded-xl p-4 text-center cursor-pointer transition bg-slate-700 hover:border-blue-500/60">
          ...
          <p class="text-xs text-slate-400 mt-0.5">Wire / ACH</p>
        </button>

        <button type="button" id="btnCrypto"
          onclick="selectMethod('crypto')"
          class="method-card border-2 border-transparent rounded-xl p-4 text-center cursor-pointer transition bg-slate-700 hover:border-yellow-500/60">
          ...
          <p class="text-sm font-bold text-slate-200">Crypto</p>
          <p class="text-xs text-slate-400 mt-0.5">BTC / ETH / USDT</p>
        </button>
      </div>
    </div>
    ...
    <form method="POST" action="withdraw.php" id="withdrawForm" class="hidden space-y-5">
      ...
      <div class="bg-slate-800 rounded-2xl p-5 space-y-4">
        ...
        <div>
          ...
          <input type="number" name="amount" id="amountInput"
            ...
            class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-yellow-500"
            placeholder="0.00">
          <p class="text-slate-500 text-xs mt-1" id="balanceHint">Available: $<?= number_format($balances['USDT'], 2) ?> USDT</p>
          <!-- Quick amount chips -->
          <div class="flex flex-wrap gap-2 mt-2">
            <button type="button" onclick="setAmount(100)"
              class="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white text-xs rounded-lg border border-slate-600 transition">$100</button>
            <button type="button" onclick="setAmount(500)"
              class="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white text-xs rounded-lg border border-slate-600 transition">$500</button>
            <button type="button" onclick="setAmount(1000)"
              class="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white text-xs rounded-lg border border-slate-600 transition">$1,000</button>
            <button type="button" onclick="setAmount(2500)"
              class="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 hover:text-white text-xs rounded-lg border border-slate-600 transition">$2,500</button>
          </div>
        </div>
      </div>
      ...
      <div id="bankFields" class="hidden bg-slate-800 rounded-2xl p-5 space-y-3">
        ...
        <div>
          ...
          <input type="text" name="bank_name" placeholder="e.g. Chase Bank"
            class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
        ...
        <div>
          <label class="block text-xs text-slate-400 mb-1">Account Number / IBAN</label>
          <input type="text" name="account_no" placeholder="Account number"
            class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-xs text-slate-400 mb-1">Routing / SWIFT / BIC (optional)</label>
      ...
      <!-- Crypto Fields -->
      <div id="cryptoFields" class="hidden bg-slate-800 rounded-2xl p-5 space-y-3">
        <h3 class="font-bold text-white text-sm flex items-center gap-2">
        ...
        <div>
          ...
          <input type="text" name="wallet_address" placeholder="Paste your receiving address"
            class="w-full bg-slate-700 border border-slate-600 text-white rounded-lg px-4 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-yellow-500">
        </div>
      </div>
    ...
    <div class="bg-slate-800 rounded-2xl overflow-hidden">
      <div class="px-5 py-4 border-b border-slate-700">
        <h2 class="font-bold text-white text-sm">Recent Withdrawals</h2>
      </div>
      <div class="divide-y divide-slate-700">
        ...
        <div class="px-5 py-3 flex items-center justify-between">
          <div>
            <p class="text-sm font-semibold text-white"><?= sanitize($w['asset_ticker']) ?></p>
...
function selectMethod(method) {
    ...
    if (method === 'bank') {
        ...
        btnBank.classList.remove('border-transparent', 'bg-slate-700');
        btnCrypto.classList.remove('border-yellow-500', 'bg-yellow-500/10');
        btnCrypto.classList.add('border-transparent', 'bg-slate-700');
        bankFields.classList.remove('hidden');
        cryptoFields.classList.add('hidden');
    } else {
        btnCrypto.classList.add('border-yellow-500', 'bg-yellow-500/10');
        btnCrypto.classList.remove('border-transparent', 'bg-slate-700');
        btnBank.classList.remove('border-blue-500', 'bg-blue-500/10');
        btnBank.classList.add('border-transparent', 'bg-slate-700');
