...
<body class="bg-slate-900 text-white min-h-screen pb-20">
  ...
  <main class="max-w-2xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="flex gap-2 flex-wrap">
      ...
      <a href="/web/public/app/markets.php?symbol=<?= urlencode($sym) ?>"
         class="px-3 py-1.5 rounded-lg text-sm font-medium transition
                <?= $sym === $selected ? 'bg-emerald-500 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600' ?>">
        <?= $sym ?>
      </a>
      <?php endforeach; ?>
    </div>
    ...
    <div class="bg-slate-800 rounded-2xl overflow-hidden" style="height:420px;">
    ...
    <div class="bg-slate-800 rounded-2xl overflow-hidden">
      <div class="px-5 py-4 border-b border-slate-700">
        <h2 class="font-bold text-white">Market Overview</h2>
      </div>
      <table class="w-full text-sm">
        <thead class="bg-slate-700/50">
          <tr>
            <th class="text-left text-slate-400 font-medium px-5 py-3">Asset</th>
            <th class="text-right text-slate-400 font-medium px-5 py-3">Price (USDT)</th>
            <th class="text-right text-slate-400 font-medium px-5 py-3">Action</th>
          </tr>
        </thead>
        <tbody>
          <?php foreach ($symbols as $sym): ?>
          ...
          <tr class="border-t border-slate-700 hover:bg-slate-700/30 transition">
            <td class="px-5 py-3">
              <div class="flex items-center gap-3">
                <div class="w-8 h-8 bg-slate-700 rounded-full flex items-center justify-center text-xs font-bold"><?= $base[0] ?></div>
                <div>
                  <p class="font-semibold text-white"><?= $base ?></p>
            ...
            <td class="px-5 py-3 text-right">
              <a href="/web/public/app/trading.php?symbol=<?= urlencode($sym) ?>"
                 class="bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 px-3 py-1 rounded-lg text-xs font-medium transition">
                Trade
              </a>
            </td>
          </tr>
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 flex justify-around py-2 z-50">
    ...
    <a href="/web/public/app/markets.php" class="flex flex-col items-center text-xs text-emerald-400 gap-1">
